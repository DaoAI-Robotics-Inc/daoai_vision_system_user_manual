...
1. 检查DL模型
    ...
    使用快速检测功能，观察3D匹配与深度学习结果。如果深度学习检测失败，请参考 :ref:`深度学习优化`。

    .. image:: images/3dmodfinder_check_result.png
        :scale: 50%

2. 检查您的点云质量
    检查是否有点云缺失，是否有点云噪声，导致对齐点云时出现严重偏差, 使用快速检测功能，观察点云是否有缺失，或者噪点。如果点云质量偏差，请参考 :ref:`相机配置优化`。
    
    .. image:: images/3dmodfinder_3d_mask_result.png
        :scale: 80%
...
物体搜索：
    物体搜索功能通过更改权重，接受度，搜索物体时长，最大物体数量等来对匹配结果进行微调。

        .. image:: images/object_finder.png
            :scale: 100%
    
    - 表面/边缘权重：表面权重和边缘权重的比例，通常情况下，如果物体边缘特征比较明显，那么可以适当的调高边缘权重。
    - 接受度：接受度越高，匹配结果越少，接受度越低，匹配结果越多。
    - 搜索物体时长：搜索物体的时间，时间越长，匹配结果越多，时间越短，匹配结果越少。
    - 最大物体数量：最大匹配的物体数量，数量越多，匹配结果越多，数量越少，匹配结果越少。
