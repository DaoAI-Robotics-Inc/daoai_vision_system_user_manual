半有序快速3D关键点 （Semi-Ordered Fast 3D Keypoint） 检测流程
===========================================================================

本章会详细介绍如何设置 半有序快速3D关键点 （Semi-Ordered Fast 3D Keypoint） 检测流程。

    .. image:: images/smallobject_overview.png
        :scale: 100%

半有序快速3D关键点 （Semi-Ordered Fast 3D Keypoint） 和大物体检测流程完全一样，除了深度学习模型不同。

半有序快速3D关键点 通常应用于 **特征点明显** 且 **特征点之间的相对位置固定** 的小物体。标注方法为:

- 每个物体都有一个多边形标注外轮廓
- 在每个多边形内，标注若干（大于3个）关键点。

    .. image:: images/smallobject_label.png
        :scale: 100%
...
-------------------
    ...
    .. image:: images/smallobject_upload_model.png
        :scale: 65%
...
1. 首先，点击拍照按钮给场景拍照，确认相机正常工作，以及确认物体位置，相机视野是否合适。
    ...
    .. image:: images/smallobject_capture.png
        :scale: 70%
...
2. 点击设置ROI, 使用窗口中的框截选出检测区域，这一步是为了移除背景，等干扰点云，只保留物体点云会出现的区域，这样可以使检测更快更准。完成ROI的调整之后再点击保存。这一步同时设置了参考系，ROI的坐标就是参考系。
    ...
    .. image:: images/smallobject_roi.png
        :scale: 70%
...
4. 然后就需要定义检测模型，点击定义模型中的设置ROI，然后在右侧点云模型中框选单一物体，然后点击保存。
    ...
    .. image:: images/smallobject_define_model_1.png
        :scale: 65%
...
同时，可以在设置参考点中选择不同的关键点或模型中心点作为原点。
    ...
    .. image:: images/smallobject_define_model_2.png
        :scale: 65%
...
7. 点击快速检测，可以使用对其关键点或深度学习结果来检测关键点识别结果。如果您的检测效果不佳，请检查1-6的步骤是否正确，更多请阅读 :ref:`视觉项目优化`。
    ...
    .. image:: images/smallobject_quick_detect.png
        :scale: 65%
