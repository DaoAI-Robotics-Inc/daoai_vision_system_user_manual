...
.. admonition:: 提示声明
    ...
    1. DaoAI机器人程序所用的计量单位为  毫米-mm, 旋转计量单位为  角度-°
...
机器人和DaoAI Vision Pilot之间以发送指令 |:left_right_arrow:| 接收程序回应 的模式交换信息，其中机器人充当客户端，DaoAI Vision Pilot充当服务器。|br|
机器人向DaoAI Vision Pilot发送请求，例如进行探测流程，DaoAI Vision Pilot在完成一系列操作后用相应的指令进行回复。|br|
所有请求和等待都是同步的（单线程），在收到前一个等待的回应之前，应确保机器人在此期间不发送任何新请求。|br|
...
一旦DaoAI Vision Pilot启动，它就会侦听TCP端口 ``6000`` 并等待，直到机器人发起连接。这是在机器人端通过打开对DaoAI Vision Pilot的IP地址和给定端口的TCP套接字来完成的。
...
DaoAI Vision Pilot 和机器人之间的指令和回应消息都是字符串数据，默认使用ASCII字符编码格式。

.. admonition:: 重要声明
    :class: important

        请求和回应消息没有固定长度，字符串信息使用","作为分隔符，使用";"作为结束符。虽然TCP/IP协议可防止数据丢失，但机器人客户端同样在接收数据流解码、分隔后会计算回应消息元素数量和预期数量，以确定消息的完整，和区分消息的边界阻止数据粘连情况。

        请求和回应消息由多个字符串元素组成。 浮点数据（如距离和角度）在发送之前，将乘以一个恒定系数 MULT = 10000 。 然后，接收端通过将接收的值除以该系数来还原该数据。

        这样做最初是为指定通讯交互的数据类型是整型（int32）准备的，浮点数据（如距离和角度）直接取整会损失精度，所以在取整之前要乘以常量系数 MULT，不过当我们现在使用字符串数据类型作为通讯交互的数据类型时，显然乘以常量系数 MULT是不必要的。
...
从机器人发送到DaoAI Vision Pilot的指令消息由以下字段（字符串）元素组成，其结构如下：
...
.. list-table:: 请求消息结构
   ...
   * - Position/位置字段
     - char[]
     - 3
     - 机器人法兰位置 (XYZ，以毫米为单位) 用右手机器人底座表示。每个元素都必须乘以MULT系数。
   * - Orientation/旋转字段
     - char[]
     - 4
     - 机器人法兰方向用右手机器人底座框架表示。方向编码和单位取决于所选的方向约定。每个元素都必须乘以 MULT系数。
   * - command/指令字段
     - char[]
     - 1
     - 请求指令。
   ...
   * - meta/标签字段
     - char[]
     - 2
     - meta_1，即消息的倒数第二个元素，应发送机器人的类型，meta_2，即消息的最后一个元素，应发送机器人协议版本。详细信息请参阅本文Meta消息说明。


所有字段都是必填的，并且必须为每个请求设置合理的值。有效载荷字段 payload 只对某写流程和指令有效。无效的字段请赋予零。

指令字段 command 可以控制DaoAI Vision Pilot执行不同的流程。下面将更详细地解释可能的指令及其对应的回应消息。
...
除姿势更新请求所使用的回应消息外，从机器人接收到DaoAI Vision Pilot的回应消息由以下字段（字符串）元素组成，其结构如下：
...
.. list-table:: 回应消息结构
   ...
   * - Position/位置字段
     - char[]
     - 3
     - 物体位置或拾取点偏移平移(XYZ，单位为毫米)，具体取决于回应状态。另请参阅更详细的指令说明。每个元素都必须除以MULT。
   * - Orientation/旋转字段 
     - char[]
     - 4
     - 物体方向或拾取点偏移旋转，具体取决于回应状态。另请参阅更详细的指令说明。编码和单位取决于所选的定向约定，并且每个元素都必须用MULT除以。
   * - payload/载荷字段
     - char[]
     - 6
     - 可选的有效载荷。编码和单位取决于所选的定向约定，并且每个元素都必须用MULT除以。
   * - status/状态字段
     - char[]
     - 1
     - 定义的状态值之一。
...
请求消息的载荷字段： |br|
payload_1代表DaoAI Vision Pilot执行task的id，payload_2预留。

回应消息的载荷字段在抓和放时，所表示的意思并不相同： |br|
payload_1会被用作基本的抓放信息交换，抓的时候payload_1 代表的是剩余物体数量：如在场景中探测到了5个物体，第一个位姿发送时payload_1 = 5；放的时候payload_1 代表的是物体的种类（在没有分类时此payload可被忽略或者用于其他用途）：如在场景中会出现5类物体，此次抓取到的是第四类物体 payload_1 = 4。
...
.. code-block:: python
    :linenos:
    :caption: DaoAI Vision Pilot回应常量
...
.. code-block:: python
    ...
    #Pose Object
    daoai_tcp_pose = [X 0.0,Y 0.0,Z 0.0,A 0.0,B 0.0,C 0.0]
    daoai_payload_1 = 0
    daoai_payload_2 = 0
    daoai_payload_3 = 0
    daoai_payload_4 = 0
...
```````````````````````
    ...
    **Pseudo-code**:
    ...
    .. code-block:: python
        ...
        def daoai_socket_open() :
            if (not socket_open(daoai_socket)):
                # Could not establish connection to the DaoAI system. Check if the IP and port are the same as DaoAI.
                halt
            end
        end
...
```````````````````````
    ...
    **Pseudo-code**:
    ...
    .. code-block:: python
        ...
        def send_robot_data():
            p_actual_pose = get_actual_pose()
            socket_send(floor(mult*p_actual_pose[0]), daoai_socket)
            socket_send(floor(mult*p_actual_pose[1]), daoai_socket)
            socket_send(floor(mult*p_actual_pose[2]), daoai_socket)
            socket_send(floor(mult*p_actual_pose[3]), daoai_socket)
            socket_send(floor(mult*p_actual_pose[4]), daoai_socket)
            socket_send(floor(mult*p_actual_pose[5]), daoai_socket)
            socket_send(floor(mult*0.0), daoai_socket)
            socket_send(daoai_r_command, daoai_socket)
            socket_send(daoai_payload_1, daoai_socket)
            socket_send(daoai_payload_2, daoai_socket)
            socket_send(DAOAI_ROBOT_TYPE, daoai_socket)
            socket_send(DAOAI_META_VERSION, daoai_socket)
    ...
    .. code-block:: python
        ...
        def recv_daoai_data():
            ...
            while wait_for_data == True:
                daoai_data = socket_read(16, daoai_socket)
                if daoai_data[0] == 16:
                    wait_for_data = False
                end
            end
            ...
            if daoai_data[15] != DAOAI_ROBOT_TYPE:
                # Pick-it is not configured to communicate with a UR robot.
            end
            if daoai_data[16] != DAOAI_META_VERSION:
                # The DaoAI interface version does not match the version of this program.
            end
...
`````````````````````````````````````````````````````````````
send_robot_data_2d(optional_z = False, z_value = 0)
`````````````````````````````````````````````````````````````
    ...
    **Pseudo-code**:
    ...
    .. code-block:: python
        ...
        def send_robot_data_2d(optional_z = False, z_value = 0):
            p_actual_pose = get_actual_plane_pose()

            if optional_z:
                socket_send(floor(mult*z_value), daoai_socket)
            else:
                socket_send(floor(mult*p_actual_pose[2]), daoai_socket)
            end
    ...
    **Pseudo-code**:
    ...
    .. code-block:: python
        ...
        def recv_daoai_data_2d():
            ...
            tool_in_plane = [daoai_data[1]/mult,daoai_data[2]/mult,daoai_data[3]/mult,daoai_data[4]/mult,daoai_data[5]/mult,daoai_data[6]/mult]
            daoai_tcp_pose = tool_in_plane
            cur_pose = get_actual_pose()
    ...
    **Pseudo-code**:
    ...
    .. code-block:: python
        ...
        def daoai_start_auto_calibration_2d():
            ...
            send_robot_data_2d()
            recv_daoai_data_2d() #wait the response that the vision started
            if (daoai_status != DAOAI_MODE_2D_AUTO_CALIBRATION):
                # DaoAI Cannot Start Auto 2d Calibration.
                return False
            end
            return True
        end
    ...
    .. code-block:: python
        ...
        def daoai_cam_config(id):
            ...
            if (daoai_status == DAOAI_SWITCH_CONFIG_FAIL): #Failed to switch config 
                # DaoAI switch camera config failed, please check your configs.
            end
            return False
        end
    ...
    .. code-block:: python
        ...
        def daoai_precision_check():
            ...
            if (daoai_status == DAOAI_PRECISION_CHECK_FAIL):
                # DaoAI drift check failed, please make sure the sticker is visible and has not moved since initial define.
                return False
            end

            if (daoai_status == DAOAI_PRECISION_CHECK_SUCCESS):
                #3d_error = daoai_payload_1
                # 3D error is daoai_payload_1
                return True
            end
...
.. code-block:: python
    ...
    #Robot Program
        ...
        If daoai_start_manual_calibration()
            # points should be setup before running the script
            MoveJ point_1
